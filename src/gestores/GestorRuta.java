...
public class GestorRuta {
	...
	public GestorRuta() 
	{
		...
		rutas = new ArrayList<Ruta>(this.listarTodas());
		this.relacionarConTrayectos(); //AAA
	//	gestorTrayecto.relacionarConLineas(); //???
	}
	public Ruta crearRuta(EstacionDeTransbordoMultimodal origen, EstacionDeTransbordoMultimodal destino, Integer distancia, Integer duracion, Integer pasajeros, EstadoRuta estado, Double costo, Integer idTrayecto, Trayecto trayecto) throws CamposIncorrectosException, SQLException, BaseDeDatosException
	{
		Ruta r = new Ruta();
	...
	public void relacionarConTrayectos() {
		...
		for(Ruta r: rutas) { 
			...
			Trayecto tray = (gestorTrayecto.getTodosTrayectos().stream().filter(t -> t.getId() == r.getIdTrayecto()).findFirst()).get(); ///????
	...
	//	
	public List<Ruta> agregarRutas(Integer idTrayecto, List<Ruta> listaTramos, Trayecto trayecto) throws CamposIncorrectosException, SQLException, BaseDeDatosException{
		List<Ruta> listaSalida = new ArrayList<Ruta>();
